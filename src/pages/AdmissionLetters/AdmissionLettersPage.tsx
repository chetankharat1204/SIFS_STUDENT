...
const ITEMS_PER_PAGE = 5; // User requested limit=5
...
export const AdmissionLettersPage: React.FC = () => {
  ...
  // Fetch Admission Letters
  const fetchLetters = async (pageNum: number) => {
    setLoading(true);
    const { data, error } = await apiService.get<any>(`/EducationAndInternship/Student/admission-letters?page=${pageNum}&limit=${ITEMS_PER_PAGE}`);

    if (data && data.success) {
      // Adjust based on expected API response structure
      // Typically: data.data.result or data.data.admissionLetters
      const list = data.data.admissionLetters || data.data.result || data.data.letters || data.data.data || [];
      ...
      toast.error(error || "Failed to fetch admission letters");
    }
    setLoading(false);
  };
  ...
  // Handle View Letter (Single details fetch)
  const handleViewLetter = async (fileUrl?: string, id?: number) => {
    // If we have file_url in list, use it directly
    if (fileUrl) {
      window.open(fileUrl, '_blank');
      return;
    }

    // Fallback: fetch details if we only have ID
    if (!id) {
      toast.error("Admission letter ID missing.");
      return;
    }

    setLoading(true);
    const { data, error } = await apiService.get<any>(`/EducationAndInternship/Student/admission-letters/${id}`);
    setLoading(false);

    if (data && data.success) {
      const details = data.data.admissionLetter || data.data.data || data.data || {};
      if (details.file_url) {
      ...
      toast.error(error || "Failed to load admission letter details.");
    }
  };

  // Handle Request Correction
  const handleRequestCorrection = (letter: AdmissionLetter) => {
    const letterId = letter.admission_letter_id || letter.id;
    setSelectedLetterId(letterId);
    setSelectedCourseId(letter.course_id);
    setCorrectionText("");
    setIsModalOpen(true);
  };

  const submitCorrection = async () => {
    if (!selectedLetterId || !correctionText.trim()) return;

    setSubmitting(true);
    const payload = {
      admission_letter_id: selectedLetterId,
      course_id: selectedCourseId,
      correction: correctionText
    };

    const { data, error } = await apiService.post<any>('/EducationAndInternship/Student/admission-letters/correction', payload);
              ...
              {loading ? (
                ...
                >
                  ...
                  <td className="py-4 px-4 whitespace-nowrap">
                    <button
                      onClick={() => handleViewLetter(r.file_url, r.id)}
