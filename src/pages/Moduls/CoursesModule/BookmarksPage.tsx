  ...
  const fetchBookmarks = async () => {
    try {
      ...
      const { data, error: apiError } = await apiService.get<any>(
        `/EducationAndInternship/Student/bookmarks?page=${page}&limit=${itemsPerPage}`
      );
    ...
    if (deleteModal.itemId) {
      try {
        ...
        const { error: apiError } = await apiService.delete(
          `/EducationAndInternship/Student/bookmarks/bookmark-delete/${deleteModal.itemId}`
        );
  ...
  if (error) {
    return (
      <div className="p-4 mb-4 text-red-700 bg-red-100 rounded-lg">
        {error}
        <button onClick={fetchBookmarks} className="ml-4 px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">Retry</button>
      </div>
    );
  }
        ...
        <table className="w-full table-auto">
          ...
          <tbody>
            {paged.map((r, index) => (
              <tr
                key={r.id}
                className={`border-b last:border-b hover:bg-gray-50 ${isDark ? "border-gray-700 hover:bg-gray-800" : "border-[#D9D9D9] hover:bg-gray-50"
                  }`}
              >
            ...
            {paged.length === 0 && (
              <tr>
                <td colSpan={5} className={`py-8 text-center ${isDark ? "text-gray-400" : "text-gray-500"}`}>No bookmarks found.</td>
              </tr>
            )}
          </tbody>
        </table>
