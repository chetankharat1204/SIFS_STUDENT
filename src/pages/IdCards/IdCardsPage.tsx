...
export const IdCardsPage: React.FC = () => {
  ...
  // Fetch ID Cards
  const fetchIdCards = async (pageNum: number) => {
    setLoading(true);
    // Directly call the generic API
    const { data, error } = await apiService.get<any>(`/EducationAndInternship/Student/idcards?page=${pageNum}`);

    if (data && data.success) {
      // Adjust based on expected API response structure
      // Typically: data.data.result or data.data.idCards or similar
      const list = data.data.studentIdCards || data.data.result || data.data.idCards || data.data || [];
      setIdCards(Array.isArray(list) ? list : []);

      // If pagination exists in response
      ...
      toast.error(error || "Failed to fetch ID cards");
    }
    setLoading(false);
  };
  ...
  // Handle View ID Card
  const handleViewIdCard = (fileUrl: string) => {
    if (fileUrl) {
  ...
  // Prepare Correction Modal
  const handleRequestCorrection = (card: IdCard) => {
    setSelectedCardId(card.id);
    setSelectedCourseId(card.course_id);
    setCorrectionText("");
    setIsModalOpen(true);
  };
  ...
  const submitCorrection = async () => {
    if (!selectedCardId || !correctionText.trim()) return;

    setSubmitting(true);
    const payload = {
      card_id: selectedCardId,
      course_id: selectedCourseId, // Sending if available, though API requirement might vary
      correction: correctionText
    };

    const { data, error } = await apiService.post<any>('/EducationAndInternship/Student/idcard-correction/correction', payload);
              ...
              {loading ? (
                ...
                >
                  <td className={`py-4 px-4 text-sm font-semibold whitespace-nowrap ${isDark ? "text-gray-300" : "text-gray-900"
                    ...
                    {r.course_name || r.courseName || "N/A"}
                  </td>
                  <td className={`py-4 px-4 text-sm font-semibold whitespace-nowrap ${isDark ? "text-gray-300" : "text-gray-900"
                    }`}>
                  ...
                  <td className="py-4 px-4 whitespace-nowrap">
                    <button
                      onClick={() => handleViewIdCard(r.file_url || "")}
            ...
            >
              <div className="flex flex-col gap-3 text-sm">
                <p className={isDark ? "text-gray-300" : "text-gray-900"}>
                  <span className="font-semibold">Course Name:</span> {r.course_name || r.courseName || "N/A"}
                </p>
                <p className={isDark ? "text-gray-300" : "text-gray-900"}>
