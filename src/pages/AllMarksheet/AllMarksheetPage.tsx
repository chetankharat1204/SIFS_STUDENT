...
type Marksheet = {
  id: number;
  marksheet_id?: number; // Sometimes ID might be different from table primary key
  course_id: number;
  course_name?: string;
  courseName?: string;
...
const ITEMS_PER_PAGE = 5; // User specified limit=5 in request example

export const AllMarksheetPage: React.FC = () => {
  ...
  // Fetch Marksheets
  const fetchMarksheets = async (pageNum: number) => {
    setLoading(true);
    // User requested limit=5
    const { data, error } = await apiService.get<any>(`/EducationAndInternship/Student/marksheets?page=${pageNum}&limit=${ITEMS_PER_PAGE}`);

    if (data && data.success) {
      // Adjust based on actual API response structure
      // Structure: data.data.data is the array
      const list = data.data.data || data.data.studentMarksheets || data.data.result || data.data.marksheets || [];
      setMarksheets(Array.isArray(list) ? list : []);
      ...
      toast.error(error || "Failed to fetch marksheets");
    }
    setLoading(false);
  };
  ...
  // Handle View Marksheet
  const handleViewMarksheet = async (id: number) => {
    // Determine the ID to use (id or marksheet_id)
    // The list usually has the ID needed for the detail view
    setLoading(true);
    const { data, error } = await apiService.get<any>(`/EducationAndInternship/Student/marksheets/${id}`);

    setLoading(false);
    if (data && data.success) {
      // Assuming response contains file_url or details
      const details = data.data.marksheet || data.data;
      if (details.file_url) {
        window.open(details.file_url, '_blank');
      } else {
        toast.error("File URL not found in details.");
      }
    } else {
      toast.error(error || "Failed to load marksheet details.");
    }
  };

  // Handle Request Correction
  const handleRequestCorrection = (sheet: Marksheet) => {
    setSelectedMarksheetId(sheet.id); // Assuming 'id' is 'marksheet_id'
    setSelectedCourseId(sheet.course_id);
    setCorrectionText("");
    setIsModalOpen(true);
  };

  const submitCorrection = async () => {
    if (!selectedMarksheetId || !correctionText.trim()) return;

    setSubmitting(true);
    const payload = {
      marksheet_id: selectedMarksheetId,
      course_id: selectedCourseId,
      correction: correctionText
    };

    const { data, error } = await apiService.post<any>('/EducationAndInternship/Student/marksheet-correction', payload);
  ...
  const handleRequestHardCopy = async (id: number) => {
    if (!window.confirm("Are you sure you want to request a hard copy of this marksheet?")) return;

    const { data, error } = await apiService.post<any>('/EducationAndInternship/Student/marksheet-hardcopy', {
      marksheet_id: id
    });
              ...
              {loading ? (
                ...
                >
                  ...
                  <td className="py-4 px-4 whitespace-nowrap">
                    <button
                      onClick={() => handleViewMarksheet(r.id)}
                  ...
                  <td className="py-4 px-4 whitespace-nowrap">
                    <button
                      onClick={() => !r.is_correction_pending && handleRequestCorrection(r)}
                      disabled={!!r.is_correction_pending}
                      className={`inline-flex items-center px-4 py-1 rounded-full font-semibold text-[12px] border transition-all ${r.is_correction_pending
                    ...
                    >
                      {r.is_correction_pending ? "Request Sent" : "Request For Correction"}
                    </button>
                  </td>
                  <td className="py-4 px-4 whitespace-nowrap">
                    <button
                      onClick={() => !r.is_hardcopy && handleRequestHardCopy(r.id)}
          ...
          {loading ? (
            ...
            >
              <div className="flex flex-col gap-3 text-sm">
                ...
                <p className={isDark ? "text-gray-300" : "text-gray-900"}>
                  <span className="font-semibold">Date:</span> {formatDate(r.date || r.created_at)}
                </p>
                ...
                <div className="flex flex-col space-y-2 mt-2">
                  ...
                  <button
                    onClick={() => !r.is_correction_pending && handleRequestCorrection(r)}
                    disabled={!!r.is_correction_pending}
                    className={`w-full px-4 py-2 text-sm rounded-full font-semibold border transition-all ${r.is_correction_pending
                      ? "bg-yellow-100 text-yellow-700 border-yellow-200 cursor-not-allowed"
