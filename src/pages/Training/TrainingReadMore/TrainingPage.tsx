// TrainingPage.tsx (Detailed Training View)
...
import type { Subject, LOS } from "../../../types/trainingTypes";
...
// Custom hook for fetching training data
const useTrainingData = (trainingId: string, subjectId?: number, losId?: number, locationState?: any) => {
  ...
  const [internalTrainingId, setInternalTrainingId] = useState(trainingId);

  // Fetch training details (Item 2)
  const fetchTrainingDetails = async (): Promise<{ success: boolean; id?: string }> => {
    try {
      const { data, error: apiError } = await apiService.get<any>(
        `/EducationAndInternship/Student/trainings/training/${trainingId}`
      );

      if (apiError) {
        console.error("Error fetching training details:", apiError);
        setError(apiError);
        return { success: false };
      }

      if (data?.success === false) {
        setError(data.message || "Training not found");
        return { success: false };
      }

      if (data?.success && data.data) {
        setTrainingDetails(data.data);
        // If we are on a student training page, prioritize using the studentTraining.id
        // which corresponds to the enrollment ID (e.g. 6188) rather than the generic training_id (e.g. 106)

        const newId = data.data.studentTraining?.id?.toString();
        if (newId) {
          setInternalTrainingId(newId);
  ...
  // Fetch subjects for the training
  const fetchSubjects = async (overrideId?: string) => {
    const targetId = overrideId || internalTrainingId;
    try {
      const { data, error: apiError } = await apiService.get<any>(
        `/EducationAndInternship/Student/trainings/training/${targetId}/subjects`
      );
      ...
      if (data?.success && data.data) {
        ...
        const transformedSubjects: Subject[] = subjectsData.map((subject: any) => ({
          ...
          training_id: subject.training_id || subject.trainingID || parseInt(trainingId),
  ...
  const fetchLOS = async (subjectId: number) => {
    try {
      const { data, error: apiError } = await apiService.get<any>(
        `/EducationAndInternship/Student/trainings/training/${internalTrainingId}/subject/${subjectId}/los`
      );
      ...
      if (data?.success && data.data) {
        ...
        const transformedLOS: LOS[] = losData.map((los: any) => ({
          ...
          training_id: los.training_id || los.trainingID || parseInt(trainingId),
          training_name: los.training_name,
  ...
  const fetchLOSContent = async (subjectId: number, losId: number, cachedLosList?: LOS[]) => {
    try {
      const { data, error: apiError } = await apiService.get<any>(
        `/EducationAndInternship/Student/trainings/training-view/${internalTrainingId}/${subjectId}/${losId}`
      );
      ...
      if (data?.success && data.data) {
        ...
        const transformedLOS: LOS = {
          ...
          training_id: losContent.training_id || losContent.trainingID || parseInt(trainingId),
          training_name: losContent.training_name,
    ...
    try {
      ...
      if (isBookmarked) {
        ...
        const { data, error: apiError } = await apiService.post<{ success: boolean; message: string }>(
          "/EducationAndInternship/Student/trainings/training-bookmark",
          payload
        );
  ...
  const markAsComplete = async () => {
    ...
    try {
      ...
      const { data, error: apiError } = await apiService.post<any>(
        "/EducationAndInternship/Student/trainings/training-complete",
        payload
      );
  ...
  const saveNote = async () => {
    ...
    try {
      ...
      const { data, error: apiError } = await apiService.post<{ success: boolean }>(
        "/EducationAndInternship/Student/trainings/training-note",
        payload
      );
  ...
  // Initialize data - Only on Training ID change
  useEffect(() => {
    const initData = async () => {
      ...
      setLoading(true);
      try {
        const detailsResult = await fetchTrainingDetails();
        if (!detailsResult.success) return;

        const effectiveId = detailsResult.id || trainingId;
        ...
        setError(err.message || "Failed to initialize training data");
      } finally {
        setLoading(false);
      }
    };
    ...
    initData();
  }, [trainingId]); // Removed subjectId/losId dependencies to prevent full reload
  ...
  return {
    trainingDetails, // Export trainingDetails
    subjects,
    modules,
    losList,
    currentLOS,
...
export default function TrainingPage({ trainingId: propTrainingId }: { trainingId?: string }) {
  ...
  const urlTrainingId = location.pathname.split('/').pop() || '';

  // Use prop if provided (from ReadMore), otherwise parse from URL
  const effectiveTrainingId = propTrainingId || urlTrainingId; // Renamed for clarity
  ...
  const {
    ...
    fetchLOS,
    fetchLOSContent,
  } = useTrainingData(effectiveTrainingId, subjectId as number | undefined, losId as number | undefined, location.state);
  ...
  // Redirect to trainings if error occurs (training not found)
  useEffect(() => {
    if ((error && error.includes("Training not found")) || (error && error.toLowerCase().includes("not found"))) {
  ...
  useEffect(() => {
    ...
    fetchModuleData();
  }, [activeSubjectId, subjects, effectiveTrainingId, subjectId, losId]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      requestAnimationFrame(() => {
        ...
        const hasSavedScroll = !!sessionStorage.getItem(`training-scroll-${effectiveTrainingId}`);
  ...
  }, [activeSubjectId, subjects.length, subjectParam, effectiveTrainingId]);
  ...
  const handleScroll = useCallback(() => {
    checkScroll();
    if (modulesRef.current) {
      sessionStorage.setItem(`training-scroll-${effectiveTrainingId}`, modulesRef.current.scrollLeft.toString());
    }
  }, [effectiveTrainingId]);
  ...
  useEffect(() => {
    if (!loading && modulesRef.current) {
      const savedScroll = sessionStorage.getItem(`training-scroll-${effectiveTrainingId}`);
      if (savedScroll !== null) {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-96">
        ...
        <span className="ml-3 text-lg font-medium text-blue-700/80">Loading training curriculum...</span>
      </div>
    );
  }
            ...
            <div className="flex-1">
              {currentTopic ? (
                <>
                  ...
                  {/* Content */}
                  <div className={`space-y-4 ${isDark ? "text-gray-300" : "text-slate-600"} training-content`}>
