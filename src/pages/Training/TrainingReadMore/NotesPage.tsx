...
import type { Note } from "../../../types/trainingTypes";
...
export default function NotesPage() {
  ...
  const fetchNotes = async () => {
    try {
      ...
      const { data, error: apiError } = await apiService.get<any>(
        "/EducationAndInternship/Student/training-note/notes"
      );
          ...
          ? notesData.map((note: any) => {
            const getName = (val: any) => {
              ...
              if (val && typeof val === 'object') {
                const name = val.name || val.title || val.subject_name || val.training_title;
                if (name) return name;
              }
              return null;
            };
            ...
            return {
              ...
              training_id: note.training_id || note.trainingID,
              training_name: getName(note.training_title) || getName(note.training_name) || getName(note.training) || "NA",
    ...
    if (deleteModal.itemId) {
      try {
        const { error: apiError } = await apiService.delete(
          `/EducationAndInternship/Student/training-note/note-delete/${deleteModal.itemId}`
        );
  ...
  const handleView = (note: Note) => {
    ...
    navigate(`/training/${note.training_id}?tab=trainings&subject=${note.subject_id}&los=${note.los_id}`, {
      state: {
