...
import type { Instructor } from "../../../types/trainingTypes";
...
export default function InstructorsPage({ trainingId: propTrainingId }: { trainingId?: string }) {
  ...
  const fetchInstructors = async () => {
    try {
      ...
      // Extract training ID from prop or URL
      const pathParts = window.location.pathname.split('/').filter(Boolean);
      const urlTrainingId = pathParts[pathParts.length - 1];
      const trainingId = propTrainingId || urlTrainingId || '100';

      const { data, error: apiError } = await apiService.get<any>(
        `/EducationAndInternship/Student/training-instructors/${trainingId}`
      );
      ...
      if (data?.success && data.data) {
        ...
        const transformedInstructors: Instructor[] = Array.isArray(instructorsData)
          ? instructorsData.map((instructor: any, index: number) => ({
            ...
            training_id: instructor.training_id,
          }))
          : [];
    ...
    <div className="mx-auto rounded-lg">
      ...
      {pagedInstructors.length === 0 && (
        <div className={`text-center py-8 ${isDark ? "text-gray-400" : "text-gray-500"
          ...
          No instructors found for this training.
        </div>
      )}
