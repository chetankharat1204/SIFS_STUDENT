...
// Define the training type for this page
interface CompleteTraining {
  id: number;
  training_id: number;
...
export default function CompleteTrainingPage({ trainingId: propTrainingId }: { trainingId?: string }) {
  const navigate = useNavigate();
  const [page, setPage] = useState(1);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [completedTrainings, setCompletedTrainings] = useState<CompleteTraining[]>([]);
  ...
  const fetchCompletedTrainings = async () => {
    try {
      ...
      // We use the propTrainingId passed from the parent component
      const trainingId = propTrainingId;

      const { data, error: apiError } = await apiService.get<any>(
        `/EducationAndInternship/Student/training-complete?page=${page}&limit=${itemsPerPage}${trainingId ? `&training_id=${trainingId}` : ""}`
      );
      ...
      if (data?.success && data.data) {
        ...
        const rawItems = rawResponse.data || rawResponse.StudentModules || rawResponse.completed_trainings || (Array.isArray(rawResponse) ? rawResponse : []);
        
        if (rawResponse.pagination) {
          setTotalPages(rawResponse.pagination.total_pages || 1);
        }

        const transformedTrainings: CompleteTraining[] = Array.isArray(rawItems)
          ? rawItems.map((item: any) => ({
            id: item.id || item.training_complete_id || item.completion_id || item.student_training_complete_id || item.student_module_id || Math.random(),
            training_id: item.training_id || item.studentTrainingID || item.student_training_id || item.trainingID,
            training_name: item.training_title || item.training_name || "NA",
        ...
        setCompletedTrainings(transformedTrainings);
      }
    } catch (err: any) {
      setError(err.message || "Failed to load completed trainings");
    } finally {
      setLoading(false);
    }
  };
    ...
    if (deleteModal.itemId) {
      try {
        ...
        const { error: apiError } = await apiService.delete(
          `/EducationAndInternship/Student/training-complete/training-complete-delete/${deleteModal.itemId}`
        );
  ...
  const handleView = (training: CompleteTraining) => {
    navigate(`/training/${training.training_id}?subject=${training.subject_id}&los=${training.los_id}&tab=trainings`, {
      state: {
        markAsCompleted: true,
        targetLosId: training.los_id
      }
    });
  };
  ...
  const paged = completedTrainings; // No slicing here, it's server-paged
